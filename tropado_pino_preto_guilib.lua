...
local function initFonts()
    ...
    print("[TROPA DO PINO PRETO] font: " .. tostring(picked))
end
...
local function fontInitCoro()
    ...
    dir = dir .. "\\TROPA DO PINO PRETO_lua"
    ...
    if gdi32 then
        ...
        print("[TROPA DO PINO PRETO] ffi/gdi32 unavailable, using system fonts")
    end
...
function Section:_widget(wd, x, y, w)
    ...
    elseif wd.kind == "custom" then
        if wd.fn then
            ...
            if not ok then print("[TROPA DO PINO PRETO] custom widget error: " .. tostring(err)) end
            local used = UI._cy - y
...
local function renderSectionAt(s, x, y, w)
    ...
    if not ok then print("[TROPA DO PINO PRETO] section '" .. tostring(s.title) .. "' error: " .. tostring(err)); return h end
    return rh
end
...
function M:_drawWatermark()
    ...
    if wm.parts.lua   then segs[#segs + 1] = nameSeg(wm.lua_name or "TROPA DO PINO PRETO") end
...
function M:_frame()
    ...
    if tab then
        ...
        if not ok then print("[TROPA DO PINO PRETO] tab '" .. tostring(tab.name) .. "' error: " .. tostring(err)) end
    end
...
function M:Build(opts)
    ...
    if not _getMouse then print("[TROPA DO PINO PRETO] WARNING: mouse position API not found -- cursor won't track") end
    ...
    callbacks.Register("Draw", function()
        ...
        if not ok then print("[TROPA DO PINO PRETO] frame error: " .. tostring(err)) end
    end)
    ...
    print(string.format("[TROPA DO PINO PRETO] guilib v%s ready: %d tabs, mouse=%s clock=%s",
